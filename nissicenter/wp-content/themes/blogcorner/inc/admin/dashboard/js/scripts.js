( function( $ ) {
    ...
    $('.twp-img-upload-button').click(function () {
        ...
        frame = wp.media({
            title: blogcorner_localize.upload_image,
            button: {
                text: blogcorner_localize.use_image
            },
    ...
    $(document).on('click', '.blogcorner-primary-notice .notice-dismiss', function (event) {
        event.preventDefault();
        jQuery.post(blogcorner_localize.ajax_url, {
            action: 'blogcorner_dismissed_handler',
            security : blogcorner_localize.dismiss_notice_nonce,
            notice: $(this).closest('.blogcorner-primary-notice').data('notice'),
        });
